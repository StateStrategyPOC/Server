...
public class GameOnDemandStartSidePolicy implements SidePolicy {
    ...
    public void apply(ServerState state, StoreAction action) {
        ...
        if (game.getCurrentPlayer().getPlayerToken().equals(castedAction.getPlayerToken())) {
            ...
            rrNotification = new RRNotification(true,null,null, drawnSectorCard, drawnObjectCard, null,null,null,null);

        } else {
            rrNotification = new RRNotification(false,null,null, drawnSectorCard, drawnObjectCard, null,null,null,null);
